...
class TestFormsLineFor(unittest.TestCase):
    """The shared helper both edit_german_notes.py and add_word.py call."""
    ...
    def test_noun_gets_its_plural(self):
        self.assertEqual(edit_german_notes.forms_line_for("Tür", "f"), "die Türen")

    def test_verb_gets_its_perfekt(self):
        self.assertEqual(edit_german_notes.forms_line_for("gehen"), "ist gegangen")

    def test_plural_only_noun_gets_nothing(self):
        self.assertIsNone(edit_german_notes.forms_line_for("Bücher", "pl"))

    def test_noun_without_plural_gets_nothing(self):
        self.assertIsNone(edit_german_notes.forms_line_for("Erbrechen", "n"))

    def test_unknown_word_gets_nothing(self):
        self.assertIsNone(edit_german_notes.forms_line_for("Xyzzy", "m"))


class TestAddWordIntegration(unittest.TestCase):
    """add_word.py is the day-to-day tool, so new words must get their forms too.

    add_word imports these names directly, so the patches target its namespace.
    """

    def setUp(self):
        patches = [
            mock.patch.object(add_word, "get_gender_from_wiktionary", GENDERS.get),
            mock.patch.object(add_word, "forms_line_for", lambda word, gender=None: {
                "Tür": "die Türen", "gehen": "ist gegangen"
            }.get(word)),
        ]
        for patch in patches:
            patch.start()
    ...
    def test_new_noun_is_created_with_its_plural(self):
        formatted, base, article = add_word.get_formatted_german_word("Tür")
        self.assertEqual(base, "Tür")
        self.assertEqual(article, "die")
        self.assertEqual(formatted, "<b>die Tür</b>" + build_forms_html("die Türen"))

    def test_new_verb_is_created_with_its_perfekt(self):
        formatted, base, article = add_word.get_formatted_german_word("gehen")
        self.assertIsNone(article)
        self.assertEqual(formatted, "<b>gehen</b>" + build_forms_html("ist gegangen"))

    def test_word_without_forms_is_unchanged(self):
        formatted, _, _ = add_word.get_formatted_german_word("Xyzzy")
        self.assertNotIn("data-anki-forms", formatted)


class TestDuplicateDetectionSurvivesFormsBlock(unittest.TestCase):
    """Regression guard: flattening de_word must not fuse the forms line into the
    headword, or add.sh stops recognising existing notes and adds duplicates."""

    def test_clean_word_drops_the_forms_block(self):
        rendered = "<b>der Hund</b>" + build_forms_html("die Hunde")
        self.assertEqual(add_word.clean_word(rendered), "der Hund")

    def test_clean_word_without_a_block_is_unaffected(self):
        self.assertEqual(add_word.clean_word("<b>der Hund</b>"), "der Hund")
        self.assertEqual(add_word.clean_word("der Hund"), "der Hund")

    def test_extract_base_word_round_trips(self):
        rendered = "<b>der Hund</b>" + build_forms_html("die Hunde")
        self.assertEqual(add_word.extract_base_word(rendered), ("Hund", "der"))

    def test_the_comparison_keys_still_match(self):
        """Mirrors find_duplicates(): the stored note and the new word must agree."""
        stored = add_word.clean_word("<b>der Hund</b>" + build_forms_html("die Hunde")).lower()
        without_article = add_word.re.sub(r'^(der|die|das)\s+', '', stored, flags=add_word.re.IGNORECASE)
        self.assertEqual(stored, "der hund")
        self.assertEqual(without_article, "hund")
